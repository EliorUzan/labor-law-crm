...
export async function getAccountingOverview(ownerUserId: string, month: AccountingMonth) {
  ...
  const [financialSummary, clientPayments, incomeRows, expenses, taxRows, trustRows, liabilities, obligations, legacyRecords, receivableRows, trustTotal] = await Promise.all([
    ...
    database.select({ id: financialRecords.id, clientId: clients.id, clientName: clients.name, matterId: financialRecords.matterId, matterTitle: matters.title, recordDate: financialRecords.recordDate, amount: financialRecords.amount, description: financialRecords.description, updatedAt: financialRecords.updatedAt }).from(financialRecords).innerJoin(clients, and(eq(clients.id, financialRecords.clientId), eq(clients.ownerUserId, ownerUserId))).leftJoin(matters, and(eq(matters.id, financialRecords.matterId), eq(matters.ownerUserId, ownerUserId), eq(matters.clientId, financialRecords.clientId))).where(and(eq(financialRecords.ownerUserId, ownerUserId), eq(financialRecords.type, "payment"), gte(financialRecords.recordDate, month.start), lt(financialRecords.recordDate, month.nextStart))).orderBy(asc(financialRecords.recordDate), asc(financialRecords.id)),
    ...
    database.select({ id: trustTransactions.id, clientId: clients.id, clientName: clients.name, matterId: trustTransactions.matterId, matterTitle: matters.title, transactionType: trustTransactions.transactionType, recordDate: trustTransactions.recordDate, amount: trustTransactions.amount, description: trustTransactions.description, documentLink: trustTransactions.documentLink, updatedAt: trustTransactions.updatedAt }).from(trustTransactions).innerJoin(clients, and(eq(clients.id, trustTransactions.clientId), eq(clients.ownerUserId, ownerUserId))).leftJoin(matters, and(eq(matters.id, trustTransactions.matterId), eq(matters.ownerUserId, ownerUserId), eq(matters.clientId, trustTransactions.clientId))).where(and(eq(trustTransactions.ownerUserId, ownerUserId), gte(trustTransactions.recordDate, month.start), lt(trustTransactions.recordDate, month.nextStart))).orderBy(asc(trustTransactions.recordDate), asc(trustTransactions.id)),
  ...
  const outgoingPayments = [...expenses.map((row) => ({ source: "expense" as const, row, recordDate: row.recordDate, description: row.description, amount: row.amount, kind: "הוצאה" })), ...taxRows.map((row) => ({ source: "tax" as const, row, recordDate: row.recordDate, description: row.description ?? row.taxType ?? (row.paymentKind === "vat" ? "מע״מ" : "מס"), amount: row.amount, kind: row.paymentKind === "vat" ? "מע״מ" : "מס" })), ...trustRows.filter((row) => row.transactionType === "release").map((row) => ({ source: "trust" as const, row, recordDate: row.recordDate, description: row.description ?? `שחרור נאמנות — ${row.clientName}`, amount: row.amount, kind: "נאמנות" }))].sort((first, second) => first.recordDate.localeCompare(second.recordDate));
